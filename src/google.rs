...
pub async fn handle_add(conn: &Connection, email: &str) {
    println!(
        "{}",
        format!("Menyiapkan proses login Google untuk akun: {}", email).blue()
    );
    ...
    match yup_oauth2::read_application_secret(&secret_path).await {
        Ok(secret) => {
            ...
            match auth_result {
                ...
                Err(e) => println!(
                    "{}",
                    format!("Gagal inisialisasi authenticator: {}", e).red()
                ),
            }
        }
        Err(e) => println!(
            "{}",
            format!("Gagal membaca client_secret.json: {}", e).red()
        ),
    }
}
...
pub async fn handle_get(_conn: &Connection, email: &str, service: &str) {
    ...
    if !token_cache_path.exists() {
        println!(
            "{}",
            format!(
                "ERROR: Sesi untuk {} belum ada. Ketik 'add {}' dulu.",
                email, email
            )
            .red()
        );
                ...
                Ok(auth) => {
                    ...
                    match auth.token(scopes).await {
                        Ok(token) => {
                            ...
                            println!(
                                "{}",
                                "Token aktif berhasil digenerate dari server Google:".green()
                            );
                            println!(
                                "{}",
                                "=======================================================".cyan()
                            ...
                            println!(
                                "{}",
                                "Silakan COPY token di atas untuk digunakan secara manual."
                                    .dimmed()
                            );
                        }
...
pub async fn handle_inject(_conn: &Connection, email: &str, service: &str) {
    println!(
        "Menyiapkan injeksi token {} untuk akun {}...",
        service.bold(),
        email.bold()
    );
            ...
            if let Ok(auth) = auth_result {
                ...
                if let Ok(token) = auth.token(scopes).await {
                    ...
                    if service.to_lowercase() == "antigravity" {
                        let agy_config =
                            config::get_config_dir().join("antigravity-cli-mock-config.json");
                        let config_content = format!(
                            r#"{{ "current_account": "{}", "access_token": "{}" }}"#,
                            email, access_token
                        );
                        ...
                        match std::fs::write(&agy_config, config_content) {
                            Ok(_) => {
                                ...
                                println!(
                                    "Sekarang Antigravity akan otomatis berjalan dengan akun: {}",
                                    email.cyan()
                                );
                            }
                            Err(e) => {
                                println!("{}", format!("Gagal menulis file config: {}", e).red())
                            }
                        }
                    } else {
                        println!(
                            "{}",
                            format!("Injeksi untuk service '{}' belum dikonfigurasi.", service)
                                .yellow()
                        );
                    }
                } else {
...
pub async fn handle_space(_conn: &Connection, email: &str) {
    println!(
        "Mengecek kapasitas Google Drive untuk akun {}...",
        email.bold()
    );
            ...
            if let Ok(auth) = auth_result {
                ...
                if let Ok(token) = auth.token(scopes).await {
                    ...
                    match res {
                        Ok(response) => {
                            if let Ok(json) = response.json::<serde_json::Value>().await {
                                if let Some(quota) = json.get("storageQuota") {
                                    let limit_str =
                                        quota.get("limit").and_then(|v| v.as_str()).unwrap_or("0");
                                    let usage_str =
                                        quota.get("usage").and_then(|v| v.as_str()).unwrap_or("0");

                                    let limit: f64 =
                                        limit_str.parse().unwrap_or(0.0) / 1_073_741_824.0;
                                    let usage: f64 =
                                        usage_str.parse().unwrap_or(0.0) / 1_073_741_824.0;
                        ...
                        Err(e) => {
                            println!("{}", format!("Gagal memanggil HTTP API Drive: {}", e).red())
                        }
                    }
                } else {
                    println!(
                        "{}",
                        "Gagal mendapatkan token (mungkin sesi expired)!".red()
                    );
                }
            }
        }
