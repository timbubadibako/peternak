...
pub fn handle_add_github(conn: &Connection, email: &str) {
    println!(
        "Membuka browser untuk generate token GitHub akun {}...",
        email.bold()
    );
    ...
    println!(
        "{}",
        "1. Pastikan kamu sudah login GitHub menggunakan akun tersebut.".cyan()
    );
    println!("{}", "2. Centang scope 'repo' dan 'workflow'.".cyan());
    println!(
        "{}",
        "3. Generate token dan copy token yang berawalan 'ghp_'.".cyan()
    );
    ...
    match conn.execute(
        ...
        Ok(_) => println!(
            "{}",
            "Token GitHub berhasil ditanamkan ke dalam database! 🚜"
                .green()
                .bold()
        ),
...
pub async fn handle_farm_github(conn: &Connection, email: &str, repo_name: &str) {
    println!(
        "Menyiapkan traktor untuk membuat repository GitHub '{}' di akun {}...",
        repo_name.bold(),
        email.bold()
    );

    let mut stmt = conn
        .prepare("SELECT github_token FROM accounts WHERE email = ?1")
        .unwrap();
    ...
    let github_token = match github_token {
        ...
        None => {
            println!(
                "{}",
                "Token GitHub belum di-set! Gunakan perintah 'add-github' dulu.".red()
            );
            return;
        }
    };
    ...
    match create_res {
        Ok(res) => {
            if res.status().is_success() {
                ...
                println!(
                    "{}",
                    format!(
                        "Gagal membuat repository di GitHub! Status/Error: {}",
                        err_body
                    )
                    .red()
                );
        ...
        Err(e) => println!(
            "{}",
            format!("Gagal mengirim request pembuatan repository: {}", e).red()
        ),
    }
}
