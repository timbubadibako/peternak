...
pub fn handle_add_vercel(conn: &Connection, email: &str) {
    println!(
        "Membuka browser untuk generate token Vercel akun {}...",
        email.bold()
    );
    ...
    println!(
        "{}",
        "1. Pastikan kamu sudah login Vercel menggunakan akun Google tersebut.".cyan()
    );
    println!(
        "{}",
        "2. Klik tombol 'Create', beri nama (misal: peternak) dan scope 'Full Account'.".cyan()
    );
    ...
    match conn.execute(
        ...
        Ok(_) => println!(
            "{}",
            "Token Vercel berhasil ditanamkan ke dalam database! 🚜"
                .green()
                .bold()
        ),
...
pub async fn handle_farm_vercel(conn: &Connection, email: &str, project_name: &str) {
    println!(
        "Menyiapkan traktor untuk deploy project Vercel '{}' di akun {}...",
        project_name.bold(),
        email.bold()
    );

    let mut stmt = conn
        .prepare("SELECT vercel_token FROM accounts WHERE email = ?1")
        .unwrap();
    ...
    let vercel_token = match vercel_token {
        ...
        None => {
            println!(
                "{}",
                "Token Vercel belum di-set! Gunakan perintah 'add-vercel' dulu.".red()
            );
            return;
        }
    };
    ...
    match create_res {
        Ok(res) => {
            if res.status().is_success() {
                ...
                println!(
                    "{} Project {} BERHASIL DIDAFTARKAN DI VERCEL!",
                    "🚀".green(),
                    project_name.bold()
                );
                ...
                println!(
                    "{}",
                    format!(
                        "Gagal membuat project di Vercel! Status/Error: {}",
                        err_body
                    )
                    .red()
                );
